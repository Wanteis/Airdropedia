...
const HomePage = () => {
  ...
  const filteredAirdrops = airdrops
    .filter(a =>
      (blockchain === 'All' || (a.blockchain && a.blockchain.toLowerCase().trim() === blockchain.toLowerCase().trim())) &&
      (cost === 'All' || (a.cost && a.cost.toLowerCase().trim() === cost.toLowerCase().trim())) &&
      (
        !search ||
    ...
    <main className="max-w-5xl mx-auto px-4 pt-10 pb-20">
      ...
      <section className="mt-16 flex flex-col items-center justify-center">
        {loading ? (
          ...
          <AirdropList airdrops={filteredAirdrops} setAirdrops={setAirdrops} refreshAirdrops={fetchAirdrops} sort={sort} />
        )}
      </section>
    </main>
  );
