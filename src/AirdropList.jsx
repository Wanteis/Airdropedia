...
const AirdropList = ({ airdrops, setAirdrops, refreshAirdrops, sort = 'latest' }) => {
  // Edit handler
  ...
  // Do not sort here; rely on the order from parent (filteredAirdrops)
  const isHome = window.location.pathname === '/';
  let displayedAirdrops = airdrops;
  let showViewMore = false;

  if (isHome) {
    displayedAirdrops = airdrops.slice(0, 12);
    showViewMore = airdrops.length > 12;
  } else if (window.location.pathname === '/airdrops') {
    displayedAirdrops = airdrops.slice(12);
  }
